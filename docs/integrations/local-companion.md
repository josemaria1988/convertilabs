...
La carga histórica requiere un pedido humano explícito y la opción `--history-from YYYY-MM-01`, junto con `--now --reason`. Por ejemplo, `npm run local -- sync-zeta --history-from 2026-01-01 --now --reason "Pedido humano de importar compras y ventas de 2026 por lotes"`. Amplía ventas y compras hasta el día actual: ventas por mes y páginas, compras mediante una consulta por cada mes autorizado, en todas sus monedas originales. Se conserva el presupuesto configurado, la única reserva diaria, el control de duplicados por identificador y la publicación de la copia completa. La opción no puede guardarse en la configuración recurrente. Si ya existe una copia completa, conserva artículos, stock, precios y referencias a maestros con su fecha anterior, verificando sus hashes; esta carga histórica actualiza únicamente facturas. Si no hay copia previa, incorpora también los demás datos requeridos por la publicación. Los informes se generan después desde Supabase; el detalle de compras conserva importes de líneas, sin afirmar totales de encabezado ni pagos que la fuente no suministra.
...
## Revisión operativa de Zeta · 10/09/2026

Estado: análisis de la sesión real de Rontil, documentación oficial, colección REST y código `91dc6e4`. No habilita nuevas escrituras. La evidencia de la cuenta y los casos de aceptación están en `.local-companion/zeta-operational-audit-20260910/README.md`, fuera de Git. Esta sección conserva las conclusiones para operar y ampliar la integración; las configuraciones concretas deben volver a comprobarse antes de utilizarlas.

| Operación | Contrato / superficie de Zeta | Estado de Convertilabs |
|---|---|---|
| Consultar artículos, stock y precios | Consultas masivas y reglas de listas | Disponible desde Supabase, con fecha/cobertura y monedas separadas |
| Compra de gastos | `RESTFacturaProveedorV1Agregar` | Emisor con revisión, tarjeta general, proyecto local y reserva por identidad fiscal estable |
| Compra de mercadería | Mismo alta, con artículos y efectos propios del comprobante | Bloqueada por el emisor de gastos; no sustituir SKU/cantidades por conceptos agrupados |
| Recibos de pago/cobro | `RESTRecibosPagosV1Save` / `RESTRecibosCobranzaV2Save` | Sin servicio de escritura; el encabezado no completa forma de pago ni aplicación a facturas |
| Venta | `RESTFacturaClienteV4Agregar` | Sin emisor operativo; el alta no equivale a emisión CFE |
| Cotización | Oportunidades con líneas y proformas configuradas | Preparación de texto posible; integración de cotización versionada y conversión pendiente |
| Cambio de precio | `RESTPreciosArticulosV2GrabarPrecioBase` | Sin servicio de escritura; requiere diferencia, confirmación y conciliación |
| Cambio de stock | Comprobantes de movimientos; no setter directo encontrado | Sin acción operativa; validar comprobante, unidad, depósito, reserva y efecto contable |

La revisión técnica y la aclaración operativa posterior de José María dejan estos criterios:

- **Proyecto local y centro externo:** Rontil no utiliza actualmente centros de costos en Zeta. El trabajo/proyecto de Convertilabs existe sin centro externo; su ausencia no bloquea el envío. El emisor no utiliza el código interno de `work_unit` ni su `external_code` genérico como centro. Sólo envía un centro cuando existe una correspondencia explícita en los metadatos específicos de centro Zeta; de lo contrario conserva el proyecto local y omite el centro ERP. No crea centros ni modifica exigencias contables de Zeta automáticamente.
- **Identidad fiscal:** la reserva duradera se identifica por proveedor Zeta exacto y serie/número normalizados, independientemente de fecha, total, moneda o tipo interno de comprobante. El fingerprint de contenido se conserva para auditoría; corregir una extracción no habilita otra alta de la misma identidad. Una coincidencia entre tipos fiscales distintos se bloquea conservadoramente para revisión. Las reservas antiguas se contrastan con su intento guardado o con una fuente sin cambios; una reserva cuya identidad no puede comprobarse bloquea el envío. No se borran reservas automáticamente. Durante el despliegue se mantiene la escritura apagada y se drenan ejecuciones anteriores: versiones antiguas y nuevas no deben exportar simultáneamente. No se observó una duplicación real durante la auditoría.
- **Tarjeta general, política confirmada por José María:** registrar compras/gastos pagados con tarjeta mediante `10 · Tarjeta Emitida`, sin exigir marca, banco, titular o un documento financiero adicional. Rontil acepta el aviso «Faltan documentos» en ese circuito. El contador concilia posteriormente contra el estado corporativo y reclasifica diferencias a Caja o Cuenta Directores cuando corresponda. Ese aviso aislado no debe bloquear la carga ni motivar cambios de configuración. Conservar la evidencia y cualquier dato de tarjeta ya informado como opcional; no inferirlos ni reclasificar automáticamente pagos personales. Registrar la compra no acredita que la conciliación de tarjeta haya terminado. La [guía oficial de tarjeta corporativa](https://zetasoftware.info/ayuda/preguntas-frecuentes/gestion-pyme/como-registrar-compras-realizadas-con-una-tarjeta-corporativa/) describe una alternativa más detallada, no un requisito adoptado por Rontil. Se mantienen revisión, importes, medio de pago real, preflight y deduplicación; efectivo sigue siendo efectivo.

**Importaciones y FIFO:** los proyectos locales sirven para reunir factura extranjera, DUA, fletes, despachante y otros documentos de una importación. Para saber el costo de las unidades vendidas hace falta además un registro de partidas de entrada, cantidades/costos y asignaciones de salida. Un proyecto por sí solo no implementa FIFO. La [herramienta FIFO de Zeta](https://zetasoftware.info/ayuda/gestion/herramientas/stock/actualizar-costos-fifo/) documenta partidas por compra y actualización de costos de líneas de venta; por eso un stock agregado por SKU no demuestra que Zeta carezca de PEPS. La [guía de importaciones](https://zetasoftware.info/ayuda/preguntas-frecuentes/gestion-pyme/como-registrar-la-importacion-y-nacionalizacion-de-mercaderia/) contempla calcular el costo final externamente e ingresarlo mediante nacionalización. Su utilización real, datos históricos y acceso por API siguen pendientes de validación. No ejecutar una regeneración de costos durante un diagnóstico. La sesión de Chrome venció al intentar revisar esa herramienta el 10/09 por la tarde; no se verificaron partidas reales de Rontil.

La [API de ventas](https://zetasoftware.info/ayuda/apis/indice-de-apis/gestion-y-contabilidad/facturas-de-clientes/) registra el comprobante, pero su firma y envío CFE se completan después en Zeta. Conservar estados separados: propuesta, alta aceptada, identificador conciliado, emisión y resultado fiscal. La colección no demuestra atomicidad por lote ni idempotencia nativa; no reenviar automáticamente tras un resultado incierto.

Se identificaron dos proformas distintas en el maestro de Rontil: 833, tipo Movimiento de Stock de Clientes, numerador de cotizaciones y sin forma de pago; 906, tipo Venta Contado, numerador de facturas y con forma de pago. Son candidatos al alta genérica de clientes, no flujos ya probados. Validar sus efectos, pendientes, numeración y conversión antes de elegir. La copia `sales` usa `QueryVentas`: no garantiza cubrir proformas de tipo movimiento de stock. No incluir una proforma como ingreso realizado por su tipo básico.

El [contrato de precios](https://zetasoftware.info/ayuda/apis/indice-de-apis/gestion-y-contabilidad/precio-base-y-precio-de-venta/) permite escribir un importe base; ambos importes en cero pueden eliminarlo. Un dato ausente no es una orden de borrado. Diferenciar cambio del precio de un SKU de cambio de una regla compartida y conservar base, moneda, IVA, valor anterior y alcance.

Para reportes integrales, los maestros de cajas/cuentas no son saldos. Hay contratos de pendientes, cuotas, movimientos de caja/bancos y asientos aún fuera del diario actual. La API de balance lee un resultado previamente generado: exige acreditar período, monedas y fecha de cálculo. Incorporar datos sólo dentro de una cobertura explícita y del presupuesto diario existente.

La actualización diaria también consulta `RESTFacturaProveedorV1Compras` una vez por cada mes ya presente en la copia de compras, incluido el actual: es una consulta masiva de encabezados, sin multiplicar llamadas por proveedor. Así observa pagos, cancelaciones y reaperturas de facturas antiguas. El saldo explícito se conserva como `_balance` en la factura publicada, con encabezado original, fecha de consulta e identidad comprobada. Si falta el encabezado se retira la evidencia de saldo de ese mes, sin suponer cero. Se mantiene el mismo presupuesto, reserva diaria y publicación atómica; no hay una segunda ejecución por este cambio. La cobertura distingue líneas actualizadas del mes actual de saldos refrescados para meses anteriores. Saldo cero significa que no había deuda en esa lectura: no acredita por sí solo fecha, medio de pago, recibo ni aplicación de un pago. La API mensual de recibos `RESTRecibosPagosV1QueryComprobantes` existe, pero todavía no forma parte de esta copia; su contrato no entrega vínculos a facturas ni formas de pago, por lo que no se deducen por semejanza de importes.

No usar los totales actuales de Dinero/Trabajos como ganancia o posición financiera completa: Dinero agrega monedas y limita registros; Trabajos suma importes brutos, puede incluir pendientes y mezclar importes originales con convertidos. Corregir agregaciones, signos, paginación y cobertura antes de interpretar esos resúmenes. El costo actual del artículo no demuestra costo histórico de una venta. Las opciones de Zeta «Tomar costo actual» e «Incluir artículos sin costo» también deben conservarse al comparar informes.

Orden de ampliación: separar proyecto local de centro ERP, cerrar identidad fiscal y validar el piloto con la política actual de tarjeta general; exponer acciones revisables con permisos equivalentes a la web; cotizaciones/precios; mercadería, recibos y ventas; cobertura financiera completa y costo imputado por trabajo/partida. Reutilizar los servicios, `party`, `work_unit`, documentos, partidas y evidencias existentes. No exponer el servicio privilegiado de exportación a un nuevo comando sin validar actor, organización, rol y confirmación de la operación. Estos criterios están documentados; no equivalen a cambios de runtime ni a una configuración nueva aplicada en Zeta.
