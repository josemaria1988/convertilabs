...
Solo ante un pedido explicito del usuario de actualizar ahora, usar `npm run local -- sync-zeta --now --reason '<motivo de la autorizacion humana>'` para adelantar la unica corrida del dia. `--now` y `--reason` son obligatorios juntos. Una solicitud de informe o falta de cobertura no autorizan este adelanto. Conserva el presupuesto, la reserva diaria y todos los controles; si ya hubo un intento ese dia, no vuelve a consultar aunque haya fallado. Una corrida adelantada hace que se omita la programada de las 18:00 de ese mismo dia. La recurrencia sigue a las 18:00 los dias siguientes. `--dry-run` permite inspeccionar tambien ese plan sin ejecutar la actualizacion.
...
Las listas de venta seleccionadas con `salesPriceLists` se calculan desde sus reglas y bases completas durante la misma sincronizacion diaria, con el mismo presupuesto de llamadas. El reporte `sales-prices` conserva importes sin IVA/con IVA y monedas por separado; son precios genericos de lista sin condiciones particulares de cliente o pago. Informar lista y fecha. No confundir el costo o nombre de una lista con su precio de venta ni prometer cobertura antes de publicar una corrida completa. `base-prices` permite omitir el articulo cuando existe una copia completa de la base explicita.
