-- PR-14 Treasury/Banco MVP: manual bank balances, vales, receivables and treasury risk.
...
begin
  ...
  update public.treasury_vales
  set current_principal = 0, status = 'closed', updated_at = now()
  where id = p_vale_id;
...
begin
  foreach v_table in array v_tables loop
    ...
    execute format('drop policy if exists "%s_select_member" on public.%I', v_table, v_table);
    execute format(
      'create policy "%s_select_member" on public.%I for select using (public.is_active_member(organization_id))',
      v_table,
      v_table
    );

    execute format('drop policy if exists "%s_insert_treasury_roles" on public.%I', v_table, v_table);
    execute format(
      'create policy "%s_insert_treasury_roles" on public.%I for insert with check (public.has_org_role(organization_id, array[''owner''::public.member_role, ''admin''::public.member_role, ''admin_processing''::public.member_role, ''accountant''::public.member_role, ''operator''::public.member_role]))',
      v_table,
      v_table
    );

    execute format('drop policy if exists "%s_update_treasury_roles" on public.%I', v_table, v_table);
    execute format(
      'create policy "%s_update_treasury_roles" on public.%I for update using (public.has_org_role(organization_id, array[''owner''::public.member_role, ''admin''::public.member_role, ''admin_processing''::public.member_role, ''accountant''::public.member_role, ''operator''::public.member_role])) with check (public.has_org_role(organization_id, array[''owner''::public.member_role, ''admin''::public.member_role, ''admin_processing''::public.member_role, ''accountant''::public.member_role, ''operator''::public.member_role]))',
      v_table,
      v_table
    );
  end loop;
